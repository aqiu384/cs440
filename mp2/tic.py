...
class Board:
    ...
    def negamax(self, depth, color, curr_score, nodes_left):
        if depth == 0 or nodes_left == 0:
            ...
            return color * curr_score, None, 0
        ...
        while True:
            try:
                ...
                self.colors[y_cross, x_cross] += my_inds
                val, move, expanded = self.negamax(depth - 1, -1 * color, curr_score + d_score, nodes_left - 1)
                val *= -1
                self.colors[y_cross, x_cross] -= my_inds

                expand_counter += (1 + expanded)
                ...
                return best_value, best_move, expand_counter

    def root_negamax(self):
        val, move, expand_counter = self.negamax(3, self.player, self.score, self.nodes_left)
        return self.player * val, move, expand_counter
    ...
    def negalphabeta(self, depth, alpha, beta, color, curr_score, nodes_left):
        ...
        while True:
            try:
                ...
                self.colors[y_cross, x_cross] += my_inds
                val, move, expanded = self.negalphabeta(depth - 1, -1 * beta, -1 * alpha, -1 * color, curr_score + d_score, nodes_left - 1)
                val *= -1
                self.colors[y_cross, x_cross] -= my_inds
    ...
    def root_negalphabeta(self):
        val, move, expand_counter = self.negalphabeta(DEPTH, BEST_NEG, BEST_POS, self.player, self.score, self.nodes_left)
        return self.player * val, move, expand_counter
